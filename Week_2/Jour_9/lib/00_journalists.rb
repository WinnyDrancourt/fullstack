...
def length(journalist)
    puts "Il y as #{journalist.length} Journalistes, dans la liste."
end

def mini(journalist)
    puts  "Le plus petit handle est : #{journalist.sort_by(&:length)[0]} !"
end

def fivers(journalist)
    five = []
    ...
    puts "Il y as #{five.length} handle de 5 character qui sont #{five}"
end

def upcase(journalist)
    upper = []
    journalist.each do |i|
        if i[0].match?(/[A-Z]/)
            upper << i
    ...
    puts "Voici les handles qui commences par une majuscule (#{upper.count}) :"
    print "#{upper}"
    puts
end

def alpha
    puts "Voici les handles par ordre alphabetique :"
    print "#{twitter.sort_by(&:downcase)}"
    puts
end

def croissant(journalist)
    puts "Voici les journalistes trié par taille de handle :"
    print "#{journalist.sort_by(&:length)}"
    puts
end

def epenser
    puts "Epenser se trouve en position :#{twitter.index ("@epenser")}"
end
...
def repartition
    ...
    count = 0
        ...
        if count !=0
            puts "Pour #{letter} lettres, il y as #{count} handle"
        end
    end
end
