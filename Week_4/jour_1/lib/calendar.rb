...
class CalendarDisplayer
	...
	def initialize
		...
		@chosen_year = Time.now.year
		display_loop
	end
	def display_loop
		user_entry = print_calendar(@chosen_year, @chosen_month)
		while (user_entry >2 || user_entry <0) do
			puts "Entrer 1 pour le mois precedent, 2 pour le mois suivant, 0 pour sortir"
		...
		when 2
			...
			puts "Erreur avec l'entree utilisateur"
		end
		...
		7.times do
			if day_counter_loop <= @nb_days
				...
				if(table_event.size == 0) then
					...
					print "|#{table_event[0].start.strftime('%H:%M')}    "
				else
					print "|         "
				end
			day_counter_loop += 1
